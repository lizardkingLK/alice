...
const refresh = vi.fn();
const push = vi.fn();
let searchParamsValue = new URLSearchParams('tab=board');
...
function openStatusRulesSection() {
  fireEvent.click(screen.getByRole('button', { name: /Transition rules/i }));
}

function clickAddStatusRule() {
  openStatusRulesSection();
...
describe('BoardDesignerWorkspace', () => {
  ...
  it('adds, renames, maps, reorders, and saves a column with one stable UUID', async () => {
    ...
    const addedName = screen.getByLabelText('Column name 1');
  ...
  it('creates an OR rule and upgrades a version 1 board to version 2', async () => {
    ...
    expect(
      screen.getByText(/Movement rules for Code Review/i)
    ).toBeInTheDocument();
    expect(
  ...
  it('edits a status transition while preserving board movement rules', async () => {
    ...
    searchParamsValue = new URLSearchParams('tab=board&boardSection=rules');
    renderDesigner(true, {
      version: '2',
      columns: customConfig.columns,
      transitions,
  ...
  it('removes a status transition rule', async () => {
    ...
    expect(
      screen.getByText(/No status transition rules are configured/i)
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'Save changes' }));
