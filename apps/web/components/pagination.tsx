...
type PaginationLayout = 'default' | 'stacked';
...
interface PaginationProps {
  ...
  /**
   * Dense sticky-footer chrome (no outer margin/border — parent supplies it).
   */
  readonly compact?: boolean;
  /**
   * `stacked`: summary on its own row above controls (narrow panels).
   * `default`: summary and controls share one responsive row.
   */
  ...
  return (
    <div
      className={cn(
        ...
        spaced ? 'w-full justify-between gap-2' : 'gap-4 sm:gap-6'
      )}
    >
...
export function Pagination({
  ...
  const classNames: string[] = ['flex flex-col'];
  if (stacked) {
    classNames.push('gap-2');
  } else {
    classNames.push('gap-3 sm:flex-row sm:items-center sm:justify-between');
  }
  ...
  if (compact) {
    if (stacked) {
      ...
      classNames.push('gap-2 sm:gap-3');
    }
  } else {
    classNames.push('border-border mt-6 gap-4 border-t pt-4 sm:gap-4');
  }
