...
| 3    | Per feature: unused Prisma GETs (~4h) + Zod isolation (~2h)                                             | **Started** (work-items, sprints, **attachments** + **worklogs** list GET; **profile** v1 Zod in `@repo/types`; work-items **mutation** Zod → `@repo/types` v1 — see [Mutation schema migration](#mutation-schema-migration-work-items-pilot)) |
