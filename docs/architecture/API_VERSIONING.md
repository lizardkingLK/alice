...
Work-items first cut lives in `packages/types/src/api/v1/work-items.ts`. Sprint and work-log v1 wire DTOs live in `packages/types/src/api/v1/sprints.ts` and `packages/types/src/api/v1/work-item-worklogs.ts`.
...
| 3    | Per feature: unused Prisma GETs (~4h) + Zod isolation (~2h)                                             | **Started** (work-items, sprints, **worklogs** list GET + v1 Zod in `@repo/types`; work-items mutation Zod stays in `workItems.schemas.ts` until web imports it) |
