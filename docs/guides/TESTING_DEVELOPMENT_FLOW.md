...
```
apps/api/
└── tests/                           # Unit tests (Vitest, node env)
    ├── work-items/
    │   └── workItems.schemas.test.ts
    ├── users/
...
> Specs live under `apps/api/tests/<domain>/` (same idea as web). Do not colocate `*.test.ts` under `apps/api/src/`.
...
| **Work Items & Backlog**   | `apps/web/app/work-items`<br>`apps/web/app/backlog`<br>`apps/web/app/work-items/_components/`                                              | Work item creation (Epic, Story, Task, Issue), hierarchy/subtasks via `parent_id`, table search/pagination, estimation, sprint allocation, backlog prioritization, and TipTap rich text descriptions. | `apps/web/tests/work-items/workItem-form.test.tsx`<br>`apps/web/tests/work-items/workItem-details.test.tsx`<br>`apps/web/tests/work-items/workItems-table.test.tsx`                                                                                                                                                                      | `apps/web/cypress/e2e/sprints.cy.ts` (work-item E2E TBD)                  |
...
| **Backend API Service**    | `apps/api/src/`<br>`apps/api/src/routes/`                                                                                                  | Express REST API server providing backend endpoints, Zod input validation, Supabase-backed notifications, and Supabase DB calls.                                                                      | `apps/api/tests/` (Vitest; domain folders under `tests/`)                                                                                                                                                                                                                                                                                | `apps/web/cypress/e2e/sprints.cy.ts`<br>`apps/web/cypress/e2e/home.cy.ts` |
