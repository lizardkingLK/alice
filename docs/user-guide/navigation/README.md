# Navigation

Find your way around Alice — sidebar, dashboard, shortcuts, and saved views.

**Audience:** All users

---

## In this topic

| Page                                            | What you'll learn                  |
| ----------------------------------------------- | ---------------------------------- |
| [Dashboard overview](./dashboard-overview.md)   | Overview widgets and layout        |
| [Charts](./charts.md)                           | Custom chart boards and widgets    |
| [Keyboard shortcuts](./keyboard-shortcuts.md)   | Speed up common actions            |
| [Favorites and views](./favorites-and-views.md) | Star pages and save filtered views |

---

## Sidebar groups

After sign-in, the left sidebar organizes the app:

| Group         | Examples                                                            |
| ------------- | ------------------------------------------------------------------- |
| **Platform**  | Overview, Charts, Backlog, Board, Work Items, My Work, Alice, Views |
| **Favorites** | Pages you starred (appears when you have at least one)              |
| **Projects**  | Projects list; **Sprints** (managers and admins)                    |
| **System**    | **Users** (admins only)                                             |
| **Account**   | Profile, Settings                                                   |
| **Help**      | Help, Docs, Roadmap                                                 |

Select any item to open that area. Collapse or expand the sidebar with the
trigger at the top or **Ctrl/⌘+B**. Alice remembers your preference (cookie) and
applies it on load for every dashboard page — Docs, Settings, and Profile no
longer force the sidebar closed.

---

## Header tools

The top bar on dashboard pages includes:

- **Breadcrumb** — where you are in the app
- **Star** — add/remove the current page as a favorite
- **Layers** — save the current page and filters as a view
- **Bell** — notification inbox
- **Alice** — open the AI chat assistant
- **Account menu** — profile and sign out

---

## Related

- [Work items](../work-items/README.md)
- [Board & planning](../board-and-planning/README.md)
