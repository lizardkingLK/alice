...
function SidebarProvider({
  ...
  useKeyboardShortcut(
    (event) => isModKey(event, SIDEBAR_KEYBOARD_SHORTCUT),
    (event) => {
...
function SidebarGroup({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar="group"
      className={cn(
        'relative flex w-full min-w-0 flex-col p-2 group-data-[collapsible=icon]:px-2 group-data-[collapsible=icon]:py-2.5',
        className
      )}
      {...props}
    />
